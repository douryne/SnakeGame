...
bool Snake::SnakeInItself(const Location& loc) const
{
...
bool Snake::SnakePos(const Location& loc) const
{
	for (int i = 0; i < nSegments; ++i)
	{
		if (segments[i].GetLocation() == loc)
...
Location Snake::GetTheHeadLocation(const Location& delta_loc) const
{
